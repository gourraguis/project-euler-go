...
func main() {
	...
	fmt.Println(largestProductInGrid(grid))
}
...
func largestProductInGrid(grid [][]int) int {
	...
	for _, elem := range grid {
		products = append(products, utils.SliceProduct(elem[0:4]))
	}
	for i := 0; i < length; i++ {
		for j := 0; j < length; j++ {
			//	left/right
			if j+3 < length {
				products = append(products, utils.SliceProduct(grid[i][j:j+4]))
			}
			// up/down
			if i+3 < length {
				products = append(products, utils.SliceProduct([]int{
					grid[i][j],
					grid[i+1][j],
					grid[i+2][j],
					grid[i+3][j],
	...
	return utils.SliceBiggest(products)
}
